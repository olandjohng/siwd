...
<body>
    <main class="contentt">
        ...
        <div class="card card-body shadow border-0 table-wrapper table-responsive">
            ...
            <table id="collectionReportTable" class="table table-hover align-items-center">
                ...
                <tbody>
                    <?php
                        // Initialize totals for each payment purpose
                        $totalTransferMeter = 0;
                        $totalChangeName = 0;
                        $totalRellocation = 0;
                        $totalRefund = 0;
                        ...
                        if ($payments && count($payments) > 0) {
                            foreach ($payments as $row) {
                                ...
                                if (strtolower($payment_purpose) === 'transfer meter') {
                                    $totalTransferMeter += $amount;
                                } elseif (strtolower($payment_purpose) === 'installation') {
                                    $totalChangeName += $amount;
                                } elseif (strtolower($payment_purpose) === 'reconnection') {
                                    $totalRellocation += $amount;
                                } elseif (strtolower($payment_purpose) === 'refund') {
                                    $totalRellocation += $amount;
                                }
                    ?>
                    <tr>
                        ...
                        <?php } else { ?>
                            ...
                            <td>0.00</td>
                        <?php } ?>
                        <td><?= strtolower($payment_purpose) === 'transfer meter' ? $amount : '0.00'; ?></td>
                        <td><?= strtolower($payment_purpose) === 'installation' ? $amount : '0.00'; ?></td>
                        <td><?= strtolower($payment_purpose) === 'reconnection' ? $amount : '0.00'; ?></td>
                        <td><?= strtolower($payment_purpose) === 'refund' ? $amount : '0.00'; ?></td>
                    </tr>
                    <?php
                            }
                        } else {
    ...
    <script>
        $(document).ready(function() {
            let table = new DataTable('#collectionReportTable', {
                dom: 'lBfrtip', // 'l' for length menu, 'B' for buttons, 'f' for search input, 'r' for processing display element, 't' for the table, 'i' for table information summary, 'p' for pagination control
                buttons: [
                    'copyHtml5',
                    'csvHtml5',
                    'excelHtml5',
